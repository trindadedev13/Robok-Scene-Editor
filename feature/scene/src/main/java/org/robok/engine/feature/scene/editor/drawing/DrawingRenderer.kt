...
fun drawRotatedSquare(
    ...
    val rotatedVertices = vertices.map { (x, y) ->
        val rotatedX = x * Math.cos(Math.toRadians(angle.toDouble())) - y * Math.sin(Math.toRadians(angle.toDouble()))
        val rotatedY = x * Math.sin(Math.toRadians(angle.toDouble())) + y * Math.cos(Math.toRadians(angle.toDouble()))
